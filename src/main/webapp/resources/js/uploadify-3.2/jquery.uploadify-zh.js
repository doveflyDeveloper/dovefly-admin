...
(function($) {
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					uploader : 'upload/attachment',  // The path to the server-side upload script
				...
				if (args[0]) {
					...
					if (args[0] == '*') {
						...
						$('#' + settings.queueID).find('.uploadify-queue-item').each(function() {
							...
							$(this).delay(100 * delay).fadeOut(300, function() {
								$(this).remove();
							});
						});
						...
						for (var n = 0; n < args.length; n++) {
							...
							$('#' + args[n]).delay(100 * n).fadeOut(300, function() {
								$(this).remove();
							});
						}
					}
					...
					$item.delay(100).fadeOut(300, function() {
						$(this).remove();
					});
				}
			});
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '所选文件添加至上传队列失败：';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('所选文件  “' + file.name + '”已在上传队列中.\n是否要替换它？');
					if (!replaceQueueItem) {
			...
			if (settings.itemTemplate == false) {
				settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\
					<div class="cancel">\
						<a class="del" href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">X</a>\
					</div>\
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							this.queueData.errorMsg += '\n所选文件数量超过限定总数量的剩余数量（' + errorMsg + '）。';
						} else {
							this.queueData.errorMsg += '\n所选文件数量超过上传队列的限定数量 （' + settings.queueSizeLimit + '）。';
						}
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件  “' + file.name + '" 超过了文件限定大小 （' + settings.fileSizeLimit + '）。';
						break;
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						this.queueData.errorMsg += '\n文件  “' + file.name + '" 是空文件。';
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件  “' + file.name + '" 不符合限定上传的文件类型 （' + settings.fileTypeDesc + '）。';
						break;
				}
			}
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			switch(errorCode) {
				...
				case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
					alert('已达到上传数量上限限制（' + errorMsg + '）。');
					errorString = '达到上传数量上限限制';
					break;
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('服务器上已经存在名为“' + file.name + '”的文件，\n是否覆盖已有文件？');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
	...
	$.fn.uploadify = function(method) {
		...
		if (methods[method]) {
			...
			$.error('方法 ' + method + ' 在 $.uploadify 中不存在。');
		}
