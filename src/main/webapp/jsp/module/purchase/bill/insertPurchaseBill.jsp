...
<title>采购订单<%=isModify ? "修改" : "新增"%></title>
<style type="text/css">
<!--
-->
</style>
<script type="text/javascript">
	...
	function save_onclick() {
		...
		if(<%=isModify%>){
			...
			var checkMsg = checkPrice();
			if (checkMsg) {
				dvConfirm("以下商品价格有变动：<br><br>" + checkMsg + "<br>您确定要继续提交吗？", 
					function() {
						alert("提交");
						//insert_onclick();
					}, 
	...
	function referenceGoods_callback(datas) {
		...
		.bind("input propertychange", function(){
			...
			$(this).closest("tr").find("td:eq(5)").html(unit_price);
	...
	function checkPrice() {
		var msg = "";
		$("#detail_list_table tr").each(function(idx){
			if (idx > 0) {
				var goods_name = $(this).find("td:eq(3)").text();
				var old_price = $(this).find("td:eq(4)").text();
				var new_price = $(this).find("td:eq(5)").text();
				
				if (old_price !== new_price) {
					msg += "【商品：" + goods_name + "】【原单价：" + old_price + "】【新单价：" + new_price + "】<br>";
				}
			}
		});
		return msg;
	...
	function changeSpec_onclick(img, goods_id, goods_name) {
		var html = "<div style='padding:10px;'>商品名称：" + goods_name + "<br>包装规格：<input type='text' id='goods_spec' name='goods_spec' value=''/></div>";
		dvPrompt(html, '设置包装规格', {
			submit: function (v, h, f) { 
			    if (f.goods_spec == '') { 
			        $.jBox.tip("请输入包装规格", 'error', { focusId: "goods_spec" }); // 关闭设置 yourname 为焦点 
			        return false; 
			    } 
			    ...
			    $.ajax({
					...
					async: false,
					dataType: "json",
					url: context_path + "/goodsWController/changeSpec/" + goods_id,
					data: {"spec": f.goods_spec},
					success: function(result){
						if(result && result.success){
							$(img).closest("tr").find("input[name='spec']").val(result.data.spec);
							$(img).closest("tr").find("input[name='spec_quantity']").trigger("input").trigger("propertychange");
							dvTip("包装规格修改成功！");
						}
					}
				});
	...
	<form id="form" name="form" method="post">
		...
		<div class="border_div" >
			<div class="header_div">
				<div class="left_div">
					<div class="table_title">采购订单<%= isModify ? "修改" : "新增"%>&nbsp;</div>
			...
			<div class="padding_2_div">
				<table class="insert_table">
					<tr>
						...
						<td class="field">
							<input type="text" name="bill_code" class="readonly" inputName="订单编号" value="<c:out value="${bean.bill_code }"/>" maxlength="50" validate=""/>
						</td>
						<td class="label"><span class="required_red">* </span>订单日期：</td>
						<td class="field">
							<input type="text" name="bill_date" class="input Wdate" inputName="订单日期" value="${fn:substring(bean.bill_date, 0, 10) }" validate="notNull"/>	
						</td>
					</tr>
					<tr>
						<td class="label"><span class="required_red">* </span>订单金额：</td>
						<td class="field">
							<input type="text" id="amount" name="amount" class="readonly" inputName="订单金额" value="<c:out value="${bean.amount }"/>" validate=""/>
						</td>
						...
						<div>
							<table id="detail_list_table" class="list_table">
								<tr>
									...
									<th width="5%">缩略图</th>
									<th width="20%">品名</th>
									<th width="10%">单价<br>（上一次采购的单价）</th>
									<th width="10%">单价<br>（以零售单位计）</th>
									<th width="35%">采购数量<br>（采购大包装数量 × 包装规格 = 零售单位数量）</th>
									<th width="10%">小计</th>
								</tr>
								...
								<tr>
									...
									<td id="#old_price"><c:out value="${detail.last_price }"/></td>
									<td id="#new_price"><c:out value="${detail.unit_price }"/></td>
									<td>
