...
<script type="text/javascript">
	...
	function query_onclick() {
		if (validateForm()){
			formHelper.jSubmit(formHelper.buildAction("orderBillReportController", "reportGroupByShop"));
		}
	}
	...
	function reportManager_onclick(manager_id) {
		...
		var url = context_path + "/orderBillReportController/reportManagerGroupByShop?" + params;
		dvOpenDialog(url, "店长进货统计", 900, 450, null);
	}
	
	/** 查询学校进货数据 */
	function reportManagerSchool_onclick(manager_id) {
		var params = "bill_date_from=${bill_date_from }&bill_date_to=${bill_date_to }&manager_id=" + manager_id;
		var url = context_path + "/orderBillReportController/reportManagerGroupBySchool?" + params;
		dvOpenDialog(url, "学校进货统计", 900, 450, null);
	}
	...
	function export_onclick() {
		...
		formHelper.submit(formHelper.buildAction("orderBillReportController", "exportGroupByShop", ids));
	}
			...
			<div class="padding_2_div">
				<table class="list_table">
					...
					<tr>
						...
						<td>
							<input type="button" name="querySchool" value="学校进货统计" class="button" onclick="reportManagerSchool_onclick('<c:out value="${bean.manager_id }"/>');"/>
							<input type="button" name="queryShop" value="店长进货统计"  class="button" onclick="reportManager_onclick('<c:out value="${bean.manager_id }"/>');"/>
						</td>
						</c:if>
					</tr>
					</c:forEach>
