...
<script type="text/javascript">
	...
	function enable_onclick() {
		...
		dvConfirm("您确定要启用这些图片吗？", 
			function() {
				formHelper.submit(formHelper.buildAction("bannerController", "enable", ids));
			}, 
			function() {
				//alert("干嘛要取消啊？");
			}
	...
	function disable_onclick() {
		...
		dvConfirm("您确定要禁用这些图片吗？", 
			function() {
				formHelper.submit(formHelper.buildAction("bannerController", "disable", ids));
			}, 
			function() {
				//alert("干嘛要取消啊？");
			}
...
<body>
	<form id="form" name="form" method="get">
			...
			<div class="padding_2_div">
				<table class="query_table">
					<tr>
						...
						<td class="field">
							<dv:select name="device" dicKeyword="DIC_BANNER_DEVICE" hasEmpty="true" defaultValue="${param.device }" />
						</td>
						<td class="label">状态：</td>
						<td class="field">
							<dv:radio name="status" dicKeyword="DIC_STATUS" hasEmpty="true" defaultValue="${param.status }" />
						</td>
					</tr>
				</table>
			</div>
		...
		<div class="border_div" >
			<div class="header_div">
				<div class="left_div">
					<div class="table_title">网站插页列表&nbsp;</div>
				</div>
				<div class="right_div">
					<div class="right_menu">
						...
						<input type="button" name="disable" value="禁用" class="button" onclick="disable_onclick();"/>
					</div>
				</div>
			</div>
			<div class="padding_2_div">
				<table class="list_table">
					...
					<tr>
						...
						<td>${status.count }</td>
						<td><img class="thumbnail" src="<c:out value="${bean.image }"/>?imageView2/0/w/150"/></td>
						<td><a href="javascript:detail_onclick('<c:out value="${bean.id }"/>')"><c:out value="${bean.title }"/></a></td>
						<td><dv:display dicKeyword="DIC_BANNER_DEVICE" value="${bean.device }"/></td>
						<td>${fn:substring(bean.start_time, 0, 10) }</td>
						<td>${fn:substring(bean.end_time, 0, 10) }</td>
