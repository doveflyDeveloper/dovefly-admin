...
<script type="text/javascript">
	...
	function enable_onclick() {
		...
		dvConfirm("您确定要启用这些数据吗？", 
			function() {
				formHelper.jSubmit(formHelper.buildAction("dictTypeController", "enable", ids));
			}, 
			function() {
				//alert("干嘛要取消啊？");
			}
	...
	function disable_onclick() {
		...
		dvConfirm("您确定要禁用这些数据吗？", 
			function() {
				formHelper.jSubmit(formHelper.buildAction("dictTypeController", "disable", ids));
			}, 
			function() {
				//alert("干嘛要取消啊？");
			}
	...
	function reload_onclick() {
		$.ajax({
			...
			async: false,
			dataType: "json",
			url: formHelper.buildAction("dictTypeController", "reload"),
			data: null,
...
<body>
	<form id="form" name="form" method="get">
			...
			<div class="padding_2_div">
				<table class="query_table">
					<tr>
						...
						<td class="field">
							<input type="text" name="keyword" class="input" maxlength="50" value="<c:out value="${param.keyword }"/>"/>
						</td>
					...
					<tr>
						...
						<td class="field">
							<input type="text" name="remark" class="input" maxlength="100" value="<c:out value="${param.remark }"/>"/>
						</td>
						<td class="label">状态：</td>
						<td class="field">
							<dv:select name="status" dicKeyword="DIC_STATUS" defaultValue="${param.status }" hasEmpty="true" attributes="inputName=''"/>
						</td>
					</tr>
				</table>
			</div>
		...
		<div class="border_div" >
			<div class="header_div">
				<div class="left_div">
					<div class="table_title">字典类型列表&nbsp;</div>
				</div>
				<div class="right_div">
					<div class="right_menu">
						<input type="button" name="reload" value="刷新缓存" class="button" onclick="reload_onclick();"/>
			...
			<div class="padding_2_div">
				<table class="list_table">
					...
					<tr>
						...
						<td>${status.count }</td>
						<td><a href="javascript:detail_onclick('<c:out value="${bean.id }"/>')"><c:out value="${bean.keyword }"/></a></td>
