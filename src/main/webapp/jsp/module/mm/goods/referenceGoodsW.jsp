...
<script type="text/javascript">
	var formHelper = new DvFormHelper("goodsWController");
			...
			<div class="padding_2_div">
				<table class="list_table">
					...
					<tr>
						<td>
						<%if("checkbox".equalsIgnoreCase(inputType)){ %>
							<input type="checkbox" name="dv_checkbox" value="<c:out value="${bean.id }"/>" warehouse_id="<c:out value="${bean.warehouse_id }"/>" warehouse_name="<c:out value="${bean.warehouse_name }"/>" goods_name="<c:out value="${bean.name }"/>" cost_price="<c:out value="${bean.cost_price }"/>" sale_price="<c:out value="${bean.sale_price }"/>" market_price="<c:out value="${bean.market_price }"/>" buy_rule="<c:out value="${bean.buy_rule }"/>" image="<c:out value="${bean.image }"/>" stock_sum="<c:out value="${bean.stock_sum }"/>"  spec="<c:out value="${bean.spec }"/>" onclick="selectCheckBox(this)"/>
						<%} else {%>	
							<input type="radio" name="dv_radio" value="<c:out value="${bean.id }"/>" warehouse_id="<c:out value="${bean.warehouse_id }"/>" warehouse_name="<c:out value="${bean.warehouse_name }"/>" goods_name="<c:out value="${bean.name }"/>" cost_price="<c:out value="${bean.cost_price }"/>" sale_price="<c:out value="${bean.sale_price }"/>" market_price="<c:out value="${bean.market_price }"/>" buy_rule="<c:out value="${bean.buy_rule }"/>" image="<c:out value="${bean.image }"/>" stock_sum="<c:out value="${bean.stock_sum }"/>" spec="<c:out value="${bean.spec }"/>" onclick="selectRadio(this)"/>
						<%}	%>		
						</td>
						<td>${status.count }</td>
