...
<script type="text/javascript">
	...
	function onSale_onclick() {
		...
		dvConfirm("您确定要上架销售这些商品吗？", 
			function() {
				formHelper.submit(formHelper.buildAction("goodsWController", "onSale", ids));
			}, 
			function() {
				//alert("干嘛要取消啊？");
			}
	...
	function offSale_onclick() {
		...
		dvConfirm("您确定要下架停售这些商品吗？", 
			function() {
				formHelper.submit(formHelper.buildAction("goodsWController", "offSale", ids));
			}, 
			function() {
				//alert("干嘛要取消啊？");
			}
	...
	/** 修改库存 */
	function manualChangeStockSum(goods_id) {
		...
		if (input_stock_sum.attr("tag") != "1") {//输入框未显示，先进行显示
			...
			$.ajax({
				...
				async: true,
				dataType: "json",
				url: context_path + "/goodsWController/manualChangeStockSum",
				data: {"goods_id":goods_id,"stock_sum":stock_sum},
	...
	function query4Sort_onclick() {
		dvOpenDialog(context_path + '/goodsWController/query4Sort?status=1', '商品排序', 900, 450, null);
	}
	...
	function exportList_onclick() {
		...
		formHelper.submit(formHelper.buildAction("goodsWController", "exportList", ids));
	}
			...
			<div class="padding_2_div">
				<table class="list_table">
					...
					<tr>
						...
						<td><a id="a_stock_sum_${bean.id }" href="javascript:void(0);" onclick="manualChangeStockSum(${bean.id });">修改库存</a></td>
					</tr>
					</c:forEach>
				</table>
			</div>
