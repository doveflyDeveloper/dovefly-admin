...
<script type="text/javascript">
	...
	function ok_onclick() {
		...
		for (var i = 0; i < sels.length; i++) {
			var obj = new Object();
			obj.id = $(sels[i]).val();
			obj.market_id = $(sels[i]).attr("market_id");
			...
			<div class="padding_2_div">
				<table class="list_table">
					...
					<tr>
						<td>
						<%if("checkbox".equalsIgnoreCase(inputType)){ %>
							<input type="checkbox" name="dv_checkbox" value="<c:out value="${bean.id }"/>" market_id="<c:out value="${bean.market_id }"/>" market_name="<c:out value="${bean.market_name }"/>" goods_name="<c:out value="${bean.name }"/>" cost_price="<c:out value="${bean.cost_price }"/>" sale_price="<c:out value="${bean.sale_price }"/>" market_price="<c:out value="${bean.market_price }"/>" buy_rule="<c:out value="${bean.buy_rule }"/>" image="<c:out value="${bean.image }"/>" stock_sum="<c:out value="${bean.stock_sum }"/>" spec="<c:out value="${bean.spec }"/>" onclick="selectCheckBox(this)"/>
						<%} else {%>	
							<input type="radio" name="dv_radio" value="<c:out value="${bean.id }"/>" market_id="<c:out value="${bean.market_id }"/>" market_name="<c:out value="${bean.market_name }"/>" goods_name="<c:out value="${bean.name }"/>" cost_price="<c:out value="${bean.cost_price }"/>" sale_price="<c:out value="${bean.sale_price }"/>" market_price="<c:out value="${bean.market_price }"/>" buy_rule="<c:out value="${bean.buy_rule }"/>" image="<c:out value="${bean.image }"/>" stock_sum="<c:out value="${bean.stock_sum }"/>" spec="<c:out value="${bean.spec }"/>" onclick="selectRadio(this)"/>
						<%}	%>		
						</td>
						<td>${status.count }</td>
