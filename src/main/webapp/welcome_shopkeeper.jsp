...
<script type="text/javascript">
	...
	function loadCityManagerInfo() {
		...
		$.ajax({
			...
			async: true,
			dataType: "json",
			url: context_path + "/reportController/reportByShopkeeper",
			data: {},
				...
				if(result){//存在
					if(result.success){
						...
						for (var i = 0; i < order_group_by_date.length; i++) {
							...
							if (buyer.name != obj.buyer_name) {
								buyer = {name: obj.buyer_name, data: [[Date.parse(obj.bill_date), obj.bill_amount]]};
								order_data_spline.push(buyer);
							} else {
								buyer.data.push([Date.parse(obj.bill_date), obj.bill_amount]);
							}
						}
						...
						for (var i = 0; i < trade_group_by_date.length; i++) {
							...
							if (seller.name != obj.seller_name) {
								seller = {name: obj.seller_name, data: [[Date.parse(obj.bill_date), obj.bill_amount]]};
								trade_data_spline.push(seller);
							} else {
								seller.data.push([Date.parse(obj.bill_date), obj.bill_amount]);
							}
						}
...
<body>
	...
	<div class="border_div" >
		<div class="header_div">
			<div class="left_div">
				<div class="table_title">本月进货订单量一览&nbsp;</div>
			</div>
			<div class="right_div">
	...
	<div class="border_div" >
		<div class="header_div">
			<div class="left_div">
				<div class="table_title">本月销售订单量一览&nbsp;</div>
			</div>
			<div class="right_div">
				<div class="right_menu"></div>
			</div>
