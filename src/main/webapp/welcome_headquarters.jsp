...
<script type="text/javascript">
	...
	function loadAdminInfo() {
		...
		$.ajax({
			...
			async: true,
			dataType: "json",
			url: context_path + "/reportController/reportByHeadquarters",
			data: {},
			success: function(result){
				if(result){//存在
					if(result.success){
						var purchase_group_by_city = result.data.purchase_group_by_city;
						var purchase_data_pie = [];
						for (var i = 0; i < purchase_group_by_city.length; i++) {
							var obj = purchase_group_by_city[i];
							purchase_data_pie.push([obj.city_name, obj.bill_amount]);
						}
						
						var purchase_group_by_date = result.data.purchase_group_by_date;
						var purchase_data_spline = [];
						var city = {};
						for (var i = 0; i < purchase_group_by_date.length; i++) {
							var obj = purchase_group_by_date[i];
							if (city.name != obj.city_name) {
								city = {name: obj.city_name, data: [[Date.parse(obj.bill_date), obj.bill_amount]]};
								purchase_data_spline.push(city);
							} else {
					    ...
					    $('#purchase_pie').highcharts({
					        ...
					        series: [{
					            type: 'pie',
					            name: '采购占比',
					            data: purchase_data_pie
					        }]
					    });
					    
					    $('#purchase_line').highcharts({
					        chart: {
					            type: 'spline'
					        },
					        title: {
...
<body>
	...
	<div class="border_div" >
		<div class="header_div">
			<div class="left_div">
				<div class="table_title">本月城市经理采购订单量一览&nbsp;</div>
			</div>
			<div class="right_div">
		...
		<div class="padding_2_div">
			<table class="detail_table">
				<tr>
					...
					<div id="purchase_pie" style="height:300px; width:98%;"></div>
					</td>
					<td class="field" style="width:70%;">
					<div id="purchase_line" style="height:300px; width:98%;"></div>
					</td>
				</tr>
			</table>
		</div>
