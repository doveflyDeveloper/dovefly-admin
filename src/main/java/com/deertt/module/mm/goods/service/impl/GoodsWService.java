 ...
 */
@Service
public class GoodsWService extends DvBaseService<IGoodsWDao, GoodsWVo, Integer> implements IGoodsWService, IGoodsWConstants {
	...
	public int addStockSum(Integer goods_id, BigDecimal quantity, BigDecimal cost_price, BigDecimal sale_price, String refer_bill_code) {
		int sum = 0;
		GoodsWVo goods = getDao().find(goods_id);
		if (goods == null) {
...
//		}
	...
	public int manualChangeStockSum(Integer goods_id, BigDecimal stock_sum) {
		int sum = 0;
		GoodsWVo goods = this.find(goods_id);
		BigDecimal change_quantity = stock_sum.subtract(goods.getStock_sum());
		if (change_quantity.intValue() != 0) {
			sum = this.changeStockSum(goods_id, stock_sum, goods.getStock_locked());
			this.addRecord(goods_id, change_quantity, stock_sum, BigDecimal.ZERO, goods.getStock_locked(), "", "用户手动修改库存值");
		}
