...
public class GoodsMDao extends DvBaseDao<GoodsMVo, Integer> implements IGoodsMDao, IGoodsMConstants {
	...
	public GoodsMVo find(Integer id) {
		return super.queryForObject(SQL_FIND_BY_ID, new Object[] { id }, new BeanPropertyRowMapper<GoodsMVo>(GoodsMVo.class));
	}

	public GoodsMVo findByCondition(String queryCondition) {
		List<GoodsMVo> vos = queryByCondition(queryCondition, null, 0, 1, true);
		return (vos != null && !vos.isEmpty()) ? vos.get(0) : null;
	}

	public int update(GoodsMVo vo) {
	...
	public int update(final GoodsMVo[] vos) {
		return super.batchUpdate(SQL_UPDATE_BY_ID, vos, new CircleVoArray<GoodsMVo>() {
			public Object[] getArgs(GoodsMVo vo) {
				return new Object[]{ vo.getCity_id(), vo.getCity_name(), vo.getMarket_id(), vo.getMarket_name(), vo.getItem_id(), vo.getBarcode(), vo.getName(), vo.getTitle(), vo.getTips(), vo.getType(), vo.getTag(), vo.getWeight(), vo.getShelf_life(), vo.getStorage_type(), vo.getProducer(), vo.getCost_price(), vo.getSale_price(), vo.getMarket_price(), vo.getLimit_coin_quantity(), vo.getSend_coin_quantity(), vo.getBuy_rule(), vo.getSpec(), vo.getCategory_id(), vo.getCategory_code(), vo.getSafe_line(), vo.getMax_limit(), vo.getCmt_point(), vo.getCmt_times(), vo.getImage(), vo.getImages(), vo.getDescription(), vo.getRemark(), vo.getSort(), vo.getStatus(), vo.getModify_by(), vo.getModify_at(), vo.getId()};
			}
		});
	...
	public List<GoodsMVo> queryByCondition(String queryCondition, String orderStr, int startIndex, int size, boolean selectAllClumn) {
	...
	public List<GoodsMVo> queryByCondition(String sql, int startIndex, int size) {
		if(size <= 0) {
			return super.query(sql, new BeanPropertyRowMapper<GoodsMVo>(GoodsMVo.class));
		} else {
			return super.query(sql, new BeanPropertyRowMapper<GoodsMVo>(GoodsMVo.class), startIndex, size); 
		}
	}
