package com.deertt.module.sys.notice.web;
...
public class NoticeController extends DvBaseController implements INoticeConstants {
	
	@Autowired
	protected INoticeService service;
	...
	public String create(HttpServletRequest request) throws Exception {
		return JSP_PREFIX + "/insertNotice";
	}
	...
	public String find(HttpServletRequest request, @PathVariable(REQUEST_ID) Integer id) throws Exception {
		...
		request.setAttribute(NoticeVo.COMMON_FILE_LIST_KEY, bean.getFiles());  //把附件放入request
		return JSP_PREFIX + "/insertNotice";
	}
	...
	public String insert(HttpServletRequest request, NoticeVo vo, RedirectAttributes attr) throws Exception {
		DvVoHelper.markCreateStamp(request, vo);
		DvVoHelper.populateCommonFiles(request, vo, null);//从request中注入附件信息
		vo.setIssue_status(NoticeVo.ISSUE_DEFAULT_STATUS);
		service.insert(vo);
	...
	public String update(HttpServletRequest request, NoticeVo vo, RedirectAttributes attr) throws Exception {
	...
	public String saveAndPublish(HttpServletRequest request, NoticeVo vo, RedirectAttributes attr) throws Exception {
		DvVoHelper.markModifyStamp(request, vo);
		DvVoHelper.populateCommonFiles(request, vo, null);//从request中注入附件信息
	...
	public String detail(HttpServletRequest request, @PathVariable(REQUEST_ID) Integer id) throws Exception {
		...
		request.setAttribute(REQUEST_BEAN, bean);
		request.setAttribute(NoticeVo.COMMON_FILE_LIST_KEY, bean.getFiles());  //把附件放入request
		return JSP_PREFIX + "/detailNotice";
	}
	...
	public String query(HttpServletRequest request) throws Exception {
		...
		List<NoticeVo> beans = service.queryByCondition(queryCondition, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());
		request.setAttribute(REQUEST_BEANS, beans);
		return JSP_PREFIX + "/listNotice";
	}
	...
	public String read(HttpServletRequest request, @PathVariable(REQUEST_ID) Integer id) throws Exception {
		NoticeVo bean = service.find(id);
		request.setAttribute(REQUEST_BEAN, bean);
		request.setAttribute(NoticeVo.COMMON_FILE_LIST_KEY, bean.getFiles());  //把附件放入request
		service.increaseReadTimes(id);
		return JSP_PREFIX + "/readNotice";
	}
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="/issue/{id}", produces={"application/json;charset=UTF-8"})
	public Object issue(HttpServletRequest request, @PathVariable(REQUEST_ID) Integer id) throws Exception {
		HandleResult result = new HandleResult();
		service.issue(id);
		result.setSuccess(true);
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="/cancelIssue/{id}", produces={"application/json;charset=UTF-8"})
	public Object cancelIssue(HttpServletRequest request, @PathVariable(REQUEST_ID) Integer id) throws Exception {
		HandleResult result = new HandleResult();
		service.cancelIssue(id);
		result.setSuccess(true);
